    ...
    <section className="py-24 md:py-32 px-8 bg-surface-container-low">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="font-headline text-3xl font-bold tracking-tight text-primary mb-4">Still need assistance?</h2>
        ...
        {state?.success ? (
          <div className="text-center bg-surface-container py-12 px-8 rounded-2xl transform transition-all duration-500 scale-100">
             ...
             <h3 className="font-headline text-2xl font-bold tracking-tight text-primary mb-2">Message Received</h3>
             <p className="font-body text-on-surface-variant max-w-sm mx-auto">{state.message}</p>
          </div>
        ) : (
          <form action={formAction} className="space-y-12">
            {state?.error && (
              <div className="bg-error-container text-on-error-container p-4 rounded-lg text-sm font-label mb-8">
                {state.error}
              </div>
            )}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div className="relative">
                <label className="block font-label text-xs uppercase tracking-widest text-on-surface-variant mb-2" htmlFor="name">Name</label>
                <input 
              ...
              {/* Email Input */}
              <div className="relative">
                <label className="block font-label text-xs uppercase tracking-widest text-on-surface-variant mb-2" htmlFor="email">Email</label>
                <input 
                  className="w-full bg-transparent border-0 border-b border-outline-variant/30 px-0 py-3 font-body text-base text-primary focus:ring-0 focus:border-primary transition-colors disabled:opacity-50 outline-none" 
                  id="email" 
                  name="email" 
            ...
            {/* Message Input */}
            <div className="relative">
              <label className="block font-label text-xs uppercase tracking-widest text-on-surface-variant mb-2" htmlFor="message">Message</label>
              <textarea 
            ...
            <div className="pt-8">
              <button 
                className="w-full md:w-auto bg-primary text-on-primary font-label text-sm uppercase tracking-widest px-12 py-5 rounded-full hover:bg-primary-fixed transition-colors duration-300 disabled:opacity-70 flex items-center justify-center min-w-[220px]" 
                type="submit"
                disabled={pending}
              >
                {pending ? (
